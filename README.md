...
GitHub Releases contains the downloadable installers: Windows uses an `.exe` installer and macOS uses an unsigned `.app` inside a ZIP. Move the extracted app to Applications, then use Finder's right-click > Open on its first launch. It does not require an Apple Developer certificate. On first run, configure a provider on that computer as described below; the installer contains no API keys, model credentials, or shared defaults.
